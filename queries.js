...
function getLatestPredictedRevenues(request, response, next) {
...
function getCurrentPredictedRevenues(request, response, next) {
    db.any("   WITH get_latest_month AS (                                                                              " +
        "        SELECT max(yyyy_mm) last_month FROM ap_financial_forecast.forecast_monthly_revenue                 " +
        "    ),                                                                                                     " +
        "     first_pred as (                                                                                       " +
        "      SELECT min(date_dim.yyyy_mm) first_pred                                                              " +
...
function getCurrentRevenues(request, response, next) {
    db.any(" SELECT                                                                                                               " +
        "     yyyy_mm,                                                                                                         " +
        "     iso_month_in_year,                                                                                               " +
        "     iso_year,                                                                                                        " +
...
function getLatestRevenues(request, response, next) {
    db.any("   with rev_current as (                                                              "+
...
function getLatestPredictedOrders(request, response, next) {
...
function getLatestOrders(request, response, next) {
    db.any("   with rev_current as (                                                              "+
        "     select max(yyyy_mm) last_rev_month from ap_financial_forecast.t_monthly_data     "+
        "   )                                                                                  "+
        "   select * from ap_financial_forecast.t_monthly_data                                 "+
...
function getCurrentOrders(request, response, next) {
    db.any(" SELECT                                                                                                               " +
        "     yyyy_mm,                                                                                                         " +
        "     iso_month_in_year,                                                                                               " +
        "     iso_year,                                                                                                        " +
...
function getCurrentPredictedOrders(request, response, next) {
    db.any("  WITH get_latest_month AS (                                                                              " +
        "      SELECT max(yyyy_mm) last_month FROM ap_financial_forecast.forecast_monthly_orders                   " +
        "  ),                                                                                                      " +
        "   first_pred as (                                                                                        " +
